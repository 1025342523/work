 ...
 * 自定义编辑条,title可点击
 */
public class CustomEditItemUnderline extends RelativeLayout {
    ...
    public CustomEditItemUnderline(Context context) {
        super(context);
        initContent(context);
    }

    public CustomEditItemUnderline(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CustomEditItem);
        initContent(context, typedArray);
    }

    public CustomEditItemUnderline(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initContent(Context context, TypedArray array) {
        contentView = LayoutInflater.from(context).inflate(R.layout.part_custom_edit_item_underline, this, false);
    ...
    public void setTitleNameOnclickListener(OnClickListener l) {
        if (tvName != null) {
